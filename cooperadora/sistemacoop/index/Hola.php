...
<div class="title_right">
    ...
    ?>
                  ...
                  ?>
                  <form action="Guardarcheck.php?idsocio=<?php echo $idss?>" method="POST">
                 ...
                 <?php
                  ...
                  while ($cuotas = mysqli_fetch_array($query_cuotas)) {
                    $query_cuotasp = mysqli_query($con,"SELECT * FROM pagodecuotas p, registrocuotas r where p.idcuota=r.idcuota and p.idsocio=$idss");
                        $resul_cuotasp= mysqli_num_rows($query_cuotasp);
                    while ($cuotasp = mysqli_fetch_array($query_cuotasp)) {

                      if($resul_cuotas >= 0){ 

                       if (($cuotas["mes"]==$cuotasp["mes"]) and ($resul_cuotasp >= 0)){                       

                          ?>
                  
                  <br><br><input name="che[]" type="checkbox"  id="checkbox" class="flat" 
                 value="<?php echo $cuotas['idcuota']?> " checked > <?php echo $cuotas["mes"]; ?>
                ...
                <br><br><input name="che[]" type="checkbox"  id="checkbox" class="flat" 
                 value="<?php echo $cuotas['idcuota']?> "> <?php echo $cuotas["mes"]; ?>
                  <?php 
                 }               
              }
            }
